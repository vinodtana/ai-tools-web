...
const RightSidebar = (props: any) => {
  ...
  const { user, aiCategories } = useAppSelector((state: any) => state.content);
    ...
    <div className="space-y-6">
      ...
      <Card className="">
        ...
        <CardContent className="space-y-3">
          {(categories || aiCategories.slice(0, 20) || popularCategories).map((category, index) => (
            <div 
              className="block group cursor-pointer"
              onClick={() => redirectToolUrl(`/ai-category-tools/${category?.category_name}`)}
            >
